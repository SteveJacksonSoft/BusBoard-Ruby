module Buses
  class BusService
    ...
    def get_times_of_next_buses_at_stop(stop_code, number_of_buses)
      ...
      buses_ordered_by_arrival_time = next_buses.sort { |bus1, bus2| bus1[:expectedArrival.to_s] <=> bus2[:expectedArrival.to_s] }
      buses_to_show = buses_ordered_by_arrival_time[0..(number_of_buses - 1)]
      arrival_times = buses_to_show.map { |bus| bus[:expectedArrival.to_s] }
      arrival_times
    end
  end
end
